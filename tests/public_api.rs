...
#[test]
fn pty_001_pseudo_console_is_an_explicit_bounded_capability() {
    ...
    assert_eq!(
        TerminalMode::PseudoConsole(size),
        TerminalMode::PseudoConsole(size)
    );
    let _: fn(&ExecutionHandle, &[u8]) -> Result<(), bolt_sandbox::ExecutionControlError> =
        ExecutionHandle::write_input;
    let _: fn(
        &ExecutionHandle,
        PseudoConsoleSize,
    ) -> Result<(), bolt_sandbox::ExecutionControlError> = ExecutionHandle::resize_pseudo_console;
}
