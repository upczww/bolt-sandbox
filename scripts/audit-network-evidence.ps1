...
$evidencePath = Join-Path $repositoryRoot 'docs\testing\network-evidence.json'

if (-not (Test-Path -LiteralPath $evidencePath)) {
    throw "Network executable evidence inventory is missing: $evidencePath"
}

$catalog = Get-Content -Raw -LiteralPath $catalogPath
$expectedIds = @(
    [regex]::Matches($catalog, '(?m)^\|\s*(NET-\d{3})\s*\|') |
        ForEach-Object { $_.Groups[1].Value }
)
$inventory = Get-Content -Raw -LiteralPath $evidencePath | ConvertFrom-Json
if ($inventory.schema_version -ne 1) {
    throw 'Network evidence inventory has an unsupported schema.'
}
...
if ($idDifference.Count -ne 0 -or
    ...
    throw 'Network evidence inventory must contain every NET-001..029 ID exactly once.'
}

$allowedStatuses = @('covered', 'partial', 'gap')
foreach ($entry in $entries) {
    if ($entry.status -notin $allowedStatuses) {
        throw "Network case $($entry.id) has invalid status '$($entry.status)'."
    }
    if ($entry.status -eq 'gap') {
        if ([string]::IsNullOrWhiteSpace([string]$entry.reason)) {
            throw "Network gap $($entry.id) requires a reason."
        }
        continue
    }
    $evidenceItems = @($entry.evidence)
    if ($evidenceItems.Count -eq 0) {
        throw "Network case $($entry.id) requires executable evidence."
    }
    foreach ($item in $evidenceItems) {
        ...
        if ([string]::IsNullOrWhiteSpace($relativePath) -or
            ...
            throw "Network case $($entry.id) has incomplete evidence metadata."
        }
        $absolutePath = Join-Path $repositoryRoot $relativePath
        if (-not (Test-Path -LiteralPath $absolutePath)) {
            throw "Network case $($entry.id) cites missing file '$relativePath'."
        }
        if ((Get-Content -Raw -LiteralPath $absolutePath).IndexOf($anchor) -lt 0) {
            throw "Network case $($entry.id) anchor '$anchor' is missing from '$relativePath'."
        }
    }
    if ($entry.status -eq 'partial' -and
        [string]::IsNullOrWhiteSpace([string]$entry.reason)) {
        throw "Partial network case $($entry.id) requires a remaining-gap reason."
    }
}
...
Write-Output 'Network executable evidence audit passed.'
...
if ($RequireComplete -and ($partial.Count -ne 0 -or $gaps.Count -ne 0)) {
    throw 'Network release evidence is incomplete.'
}
